        ...
        <div key={index} className={ index !== newStep.length-1 ? "w-full flex items-center": "flex items-center"}>
            <div className='relative flex flex-col items-center text-teal-600'>
                ...
                py-3 ${step.selected
                    ...
                    {/*Display stepper number */}
                    {step.completed ?(
                        <span className='text-white font-bold text-xl'>✔</span>
                    ):(index + 1)} 
                </div>
                <div className={`absolute text-center mt-16 w-32 text-xs font-medium uppercase ${step.highlighted ? "text-grey-900":"text-gray-400"}`}>
                    {/*Display stepper discription*/}
                    {step.description}
                </div>
            </div>
